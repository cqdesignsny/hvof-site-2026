...
export function renderMarkdown(md: string): string {
  ...
  for (const raw of lines) {
    ...
    if (h) {
      ...
      out.push(`<h${level} class="font-display font-semibold tracking-tight mt-4 first:mt-0">${inline(h[2])}</h${level}>`);
      continue;
    }
